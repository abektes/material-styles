"""Style 14: Bauhaus Photogram & Typofoto (László Moholy-Nagy (Typofoto 1925), Herbert Bayer)."""
import math
...
class BauhausTypofotoStyle(BaseStyle):
    metadata = StyleMetadata(
        id="style_bauhaus_typofoto",
        number=14,
        name="Bauhaus Photogram & Typofoto",
        lineage="László Moholy-Nagy (Typofoto 1925), Herbert Bayer",
        substrate_hex="#EDE8DF",
        substrate_rgb=SUB_BROMO_CHLORIDE,
        inks=[INK_EMULSION_BLACK, INK_BAUHAUS_RED],
        gate_range=(35, 55),
        description="Camereless optical photogram, lens refraction rings, spiral spring, constructivist red focal ray",
        slug="14-bauhaus-typofoto",
        default_content=PosterContent(
            headline="typofoto",
            subhead="licht + raum + bewegung // bauhaus 1925",
        ),
    )
    ...
    def render(
        ...
        headline = content.get_slot("headline", "typofoto")
        subhead = content.get_slot("subhead", "licht + raum + bewegung // bauhaus 1925")
        dx, dy = P.drift
